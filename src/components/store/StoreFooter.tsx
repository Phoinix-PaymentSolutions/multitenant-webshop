// src/components/store/StoreFooter.tsx
'use client';

import React from 'react';
import dynamic from 'next/dynamic';
import { OperatingHoursDisplay } from './OperatingHoursDisplay';
import { OperatingHoursTakeaway, OperatingHoursDelivery } from '@/types';

// Dynamically import Map component to avoid SSR issues
const Map = dynamic(() => import('./Map'), { 
  ssr: false,
  loading: () => <div className="w-full h-[250px] bg-gray-100 rounded-lg animate-pulse" />
});

interface StoreFooterProps {
  store: {
    name: string;
    description?: string;
    contactEmail?: string;
    contactPhone?: string;
    latitude?: number;
    longitude?: number;
    placeId?: string;
    contact?: {
      address?: string;
      postalCode?: string;
      email?: string;
      phone?: string;

    };
    socialMedia?: {
      instagram?: string;
      facebook?: string;
      tiktok?: string;
    };
    operatingHoursTakeaway?: OperatingHoursTakeaway;
    operatingHoursDelivery?: OperatingHoursDelivery;
  };
  currentPage: 'store' | 'contact' | 'info' | 'reviews';
  onPageChange: (page: 'store' | 'contact' | 'info' | 'reviews') => void;
}

export const StoreFooter = ({ store }: StoreFooterProps) => {
  const email = store.contactEmail || store.contact?.email;
  const phone = store.contactPhone || store.contact?.phone;
  const address = store.contact?.address;
  const postalCode = store.contact?.postalCode;

  const scrollToTop = () => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  return (
    <footer className="w-full bg-white border-t border-gray-200 mt-24">
      {/* Powered by Maal-Tijd - Top Section */}
      <div className="container mx-auto px-4 pt-8 pb-4">
        <div className="text-center">
          <p className="text-gray-400 text-sm">
            Powered by <span className="font-semibold text-gray-600">Maal-Tijd</span>
          </p>
        </div>
      </div>

      <div className="container mx-auto px-4 pb-12">
        {/* Main Content - Two Columns */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-8">
          
          {/* Opening Hours - Prominent Left Column */}
          <div>
            <h3 className="text-2xl font-bold text-gray-900 mb-6">Openingstijden</h3>
            {store?.operatingHoursTakeaway || store?.operatingHoursDelivery ? (
              <OperatingHoursDisplay 
                operatingHoursTakeaway={store.operatingHoursTakeaway} 
                operatingHoursDelivery={store.operatingHoursDelivery}
              />
            ) : (
              <div className="bg-gray-50 rounded-lg p-8 text-center">
                <p className="text-gray-500">Openingstijden niet beschikbaar</p>
              </div>
            )}
          </div>

          {/* Contact & Social - Right Column */}
          <div className="space-y-8">
            {/* Contact Information */}
            {(email || phone || address) && (
              <div>
                <h3 className="text-2xl font-bold text-gray-900 mb-6">Contact</h3>
                <div className="space-y-4">
                  {email && (
                    <a
                      href={`mailto:${email}`}
                      className="flex items-center gap-3 text-gray-700 hover:text-gray-900 transition-colors"
                    >
                      <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                      <span>{email}</span>
                    </a>
                  )}

                  {phone && (
                    <a
                      href={`tel:${phone}`}
                      className="flex items-center gap-3 text-gray-700 hover:text-gray-900 transition-colors"
                    >
                      <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                      </svg>
                      <span>{phone}</span>
                    </a>
                  )}

                  {address && (
                    <div className="flex items-start gap-3 text-gray-700">
                      <svg className="w-5 h-5 text-gray-400 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                      <div>
                        <p>{address}</p>
                        {postalCode && <p className="text-gray-600">{postalCode}</p>}
                      </div>
                    </div>
                  )}
                </div>
              </div>
            )}

            {/* Social Media */}
            {(store.socialMedia?.instagram || store.socialMedia?.facebook || store.socialMedia?.tiktok) && (
              <div>
                <h4 className="font-semibold text-gray-900 mb-4">Volg ons</h4>
                <div className="flex gap-3">
                  {store.socialMedia.instagram && (
                    <a
                      href={store.socialMedia.instagram}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center w-11 h-11 bg-gradient-to-br from-purple-600 via-pink-600 to-orange-600 text-white rounded-lg hover:scale-110 transition-transform"
                      aria-label="Instagram"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                      </svg>
                    </a>
                  )}
                  {store.socialMedia.facebook && (
                    <a
                      href={store.socialMedia.facebook}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center w-11 h-11 bg-blue-600 text-white rounded-lg hover:scale-110 transition-transform"
                      aria-label="Facebook"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                      </svg>
                    </a>
                  )}
                  {store.socialMedia.tiktok && (
                    <a
                      href={store.socialMedia.tiktok}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center w-11 h-11 bg-black text-white rounded-lg hover:scale-110 transition-transform"
                      aria-label="TikTok"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/>
                      </svg>
                    </a>
                  )}
                </div>
              </div>
            )}

            {/* Map - Shows if lat/lng are available */}
            {store.latitude && store.longitude && (
              <div>
                <h4 className="font-semibold text-gray-900 mb-4">Locatie</h4>
                <Map lat={store.latitude} lng={store.longitude} />
              </div>
            )}
          </div>
        </div>
        
        {/* Bottom Bar */}
        <div className="border-t border-gray-200 pt-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-gray-600 text-sm">
              © {new Date().getFullYear()} <span className="font-semibold">{store?.name}</span>
            </p>
            
            {/* Scroll to Top Button */}
            <button
              onClick={scrollToTop}
              className="flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors text-sm group"
              aria-label="Terug naar boven"
            >
              <span>Terug naar boven</span>
              <svg 
                className="w-4 h-4 group-hover:-translate-y-1 transition-transform" 
                fill="none" 
                stroke="currentColor" 
                viewBox="0 0 24 24"
              >
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18" />
              </svg>
            </button>
          </div>
        </div>
      </div>
    </footer>
  );
};